...
public class FeignRequestInterceptor implements RequestInterceptor {
	...
	public void apply(RequestTemplate template) {
		log.info("[OAuth2FeignRequestInterceptor] start interceptor");
		if (template.headers().containsKey(AUTHORIZATION_HEADER)) 
			log.warn("The Authorization token has been already set");
		else {
			log.info("[OAuth2FeignRequestInterceptor] Constructing Header {} for Token {}", AUTHORIZATION_HEADER, BEARER_TOKEN_TYPE);
